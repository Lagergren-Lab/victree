...
class VarTreeJointDist(JointDist):
    ...
    def elbo_observations(self):
        ...
        E_CZ_log_tau = torch.einsum("umi, nu, nu, m ->", qC, qZ, E_log_tau, (~nan_mask).float()) if type(self.mt) is qMuTau\
            else torch.einsum(
            "umi, nu, m ->", qC, qZ, E_log_tau, (~nan_mask).float())  # TODO: possible to replace einsum with M * torch.sum(E_log_tau)?
        E_CZ_tau_y2 = torch.einsum("umi, nu, nu, mn ->", qC, qZ, E_tau, y ** 2) if type(
            self.mt) is qMuTau else torch.einsum("umi, nu, , mn ->", qC, qZ, E_tau, y ** 2)
        E_CZ_mu_tau_cy = torch.einsum("umi, nu, nu, mn, mni ->", qC, qZ, E_mu_tau, y, c.expand(M, N, A))
        E_CZ_mu2_tau_c2 = torch.einsum("umi, nu, nu, i, m ->", qC, qZ, E_mu2_tau, c2, (~nan_mask).float())
...
class FixedTreeJointDist(JointDist):
    ...
    def elbo_observations(self):
        ...
        if isinstance(self.mt, qMuTau):
            ...
            c2 = c ** 2
            M, N = y.shape
            E_CZ_log_tau = torch.einsum("umi, nu, nu, m ->", qC, qZ, E_log_tau, (~nan_mask).float()) \
