...
```{r}
setwd("/Users/zemp/phd/scilife/coPyTree/")
library(rhdf5)
h5file_path <- file.path(getwd(), "tests/test_output/checkpoint_k3a3n30m5.h5")
f <- H5Fopen(h5file_path)
```
...
checkpoint <- list()
by(h5ls(f), seq_len(nrow(h5ls(f))), function(row, checkpoint) {
  print(row)
  if (is.null(checkpoint$`row$group`)) {
    checkpoint$`row$group` <- list()
  }
  checkpoint$`row$group`$`row$name` <- f$`row$group`$`row$name` 
